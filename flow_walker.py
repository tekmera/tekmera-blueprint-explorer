...
class FlowWalker:
        ...
        for i, module in enumerate(flow_modules):
            ...
            if filter_card:
                ...
                if next_modules:
                    ...
                    filter_card.description = f"Conditions that must be met for Module {next_modules[0]} to execute"
                self.flow_steps.append(filter_card)
            
            # If this is a router, record branch points
            if step.is_router and module.get('routes'):
                self.branch_points[current_step_index] = {
                    'branches': [],
                    'module_data': module
                }
                ...
                for route in module['routes']:
                    branch_info = {
                        ...
                        'first_module_id': route.get('flow', [{}])[0].get('id') if route.get('flow') else None
                    }
                    self.branch_points[current_step_index]['branches'].append(branch_info)
    
    def _create_flow_step(self, module: Dict[str, Any], next_modules: List[int], 
                         previous_modules: List[int], depth: int) -> FlowStep:
    ...
    def _generate_module_description(self, module: Dict[str, Any]) -> str:
        ...
        if 'search' in module_type:
            ...
            return f"Route execution to {route_count} different path{'s' if route_count != 1 else ''}"
    ...
    def _extract_input_mappings(self, module: Dict[str, Any]) -> List[str]:
        ...
        for key, value in list(mapper.items())[:5]:  # Limit to first 5
            if isinstance(value, dict):
                ...
                if 'value' in value:
                    mappings.append(f"{key}: {str(value['value'])[:30]}{'...' if len(str(value['value'])) > 30 else ''}")
                elif 'name' in value:
    ...
    def _extract_filter_conditions(self, module: Dict[str, Any]) -> List[str]:
        ...
        if 'filter' in parameters:
            ...
            if isinstance(filter_value, dict) and 'value' in filter_value:
                conditions.append(str(filter_value['value'])[:50] + '...' if len(str(filter_value['value'])) > 50 else str(filter_value['value']))
            elif isinstance(filter_value, str):
                conditions.append(filter_value[:50] + '...' if len(filter_value) > 50 else filter_value)
        
        # Check for other conditional parameters
        for key, value in parameters.items():
            if 'condition' in key.lower() or 'where' in key.lower():
                if isinstance(value, dict) and 'value' in value:
                    conditions.append(f"{key}: {str(value['value'])[:40]}{'...' if len(str(value['value'])) > 40 else ''}")
    ...
    def _extract_key_parameters(self, module: Dict[str, Any]) -> Dict[str, Any]:
        ...
        important_keys = [
            'objType', 'objId', 'action', 'method', 'url', 'query', 
            'recordType', 'operation', 'value', 'field', 'limit'
        ]
            ...
            if 'endpoint' in key.lower() or 'api' in key.lower():
                if isinstance(value, dict) and 'value' in value:
                    ...
                    if ref_value and not ref_value.startswith('http'):
                        references.append(f"API: {ref_value[:30]}{'...' if len(ref_value) > 30 else ''}")
    ...
    def _extract_search_criteria(self, module: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in parameters.items():
            if key.lower() in ['query', 'search', 'where', 'filter']:
                if isinstance(value, dict) and 'value' in value:
                    search_fields.append(f"{key}: {str(value['value'])[:40]}{'...' if len(str(value['value'])) > 40 else ''}")
    ...
    def _extract_operation_details(self, module: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'create' in module_type:
            ...
            for key, value in list(mapper.items())[:4]:  # First 4 fields
                if isinstance(value, dict) and 'value' in value:
                    create_fields.append(f"{key}: {str(value['value'])[:30]}{'...' if len(str(value['value'])) > 30 else ''}")
                elif isinstance(value, str):
        ...
        # HTTP requests
            ...
            if 'url' in parameters:
                ...
                if isinstance(url, dict) and 'value' in url:
                    operation_info['endpoint'] = str(url['value'])[:50] + '...' if len(str(url['value'])) > 50 else str(url['value'])
    ...
    def _extract_webhook_config(self, module: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'url' in parameters:
            url = parameters['url']
            if isinstance(url, dict) and 'value' in url:
                webhook_info['webhook_url'] = str(url['value'])[:40] + '...' if len(str(url['value'])) > 40 else str(url['value'])
        ...
        if 'transform' in module_type or 'tools' in module_type:
            ...
            if 'formula' in parameters:
                ...
                if isinstance(formula, dict) and 'value' in formula:
                    transformations.append(f"Formula: {str(formula['value'])[:40]}{'...' if len(str(formula['value'])) > 40 else ''}")
            ...
            for key, value in mapper.items():
                if isinstance(value, dict):
                    if 'function' in value:
                        ...
                        transformations.append(f"{key}: {expr}{'...' if len(str(value['expression'])) > 30 else ''}")
    ...
    def _create_filter_card_if_needed(self, module: Dict[str, Any], index: int) -> Optional[FlowStep]:
    ...
    def _extract_comprehensive_filter_info(self, module: Dict[str, Any]) -> Dict[str, Any]:
        ...
        conditional_params = []
        for key, value in parameters.items():
            if any(word in key.lower() for word in ['condition', 'criteria', 'constraint', 'requirement']):
                if isinstance(value, dict) and 'value' in value:
        ...
        date_keywords = ['date', 'time', 'created', 'updated', 'modified', 'entryDate', 'plannedCompletionDate']
        date_filters = []
...
[dim]Total modules to explore: {len(self.flow_steps)}[/dim]"""
        ...
        panel = Panel(
            welcome_text,
            title="🎥 Live Scenario Walkthrough",
            border_style="blue",
            expand=False
        )
        ...
        if not self.keyboard_available:
            self.console.print("[yellow]Arrow key navigation not available on this system. Using traditional menu.[/yellow]")
    ...
    def _display_current_step(self, step: FlowStep):
        ...
        progress_bar = "█" * (self.current_step_index + 1) + "░" * (len(self.flow_steps) - self.current_step_index - 1)
        ...
        if step.variables_used:
            ...
            var_display = ", ".join([f"{{{{[bright_yellow]{var}[/bright_yellow]}}}}" for var in step.variables_used[:4]])
        ...
        if step.search_criteria:
            ...
            if 'order_by' in step.search_criteria:
                content += f"  • [bright_cyan]Order By:[/bright_cyan] {step.search_criteria['order_by']}\n"
        ...
        if step.webhook_config:
            ...
            if 'webhook_url' in step.webhook_config:
                content += f"  • [bright_green]URL:[/bright_green] {step.webhook_config['webhook_url']}\n"
            if 'method' in step.webhook_config:
                content += f"  • [bright_green]Method:[/bright_green] {step.webhook_config['method']}\n"
            if 'expects' in step.webhook_config:
                content += f"  • [bright_green]Expects:[/bright_green] {step.webhook_config['expects']}\n"
        ...
        if step.is_router and self.current_step_index in self.branch_points:
            ...
            for i, branch in enumerate(branches, 1):
                ...
                if branch['filter'] and branch['filter'] != 'No filter':
                    filter_preview = branch['filter'][:40] + '...' if len(branch['filter']) > 40 else branch['filter']
    ...
    def _display_filter_card(self, step: FlowStep):
        ...
        if step.next_modules:
            content += f"[bright_blue]Controls execution of Module {step.next_modules[0]}[/bright_blue]\n"
        ...
        if 'condition_name' in filter_details:
            content += f"[dim]Condition Name:[/dim] [cyan]{filter_details['condition_name']}[/cyan]\n\n"
    ...
    def _get_user_action(self) -> str:
        ...
        if self.current_step_index in self.branch_points:
            ...
            choices.append({"name": f"🔀 Choose branch path ({branch_count} options)", "value": "next"})
        elif self.current_step_index < len(self.flow_steps) - 1:
    ...
    def _show_detailed_view(self, step: FlowStep):
        ...
        if step.mapper:
            ...
            syntax = Syntax(
                json.dumps(step.mapper, indent=2),
                "json",
                theme="monokai",
                line_numbers=True
            )
...
[dim]You now have a complete understanding of how this scenario flows from start to finish.[/dim]"""
    ...
    def _handle_router_branching(self) -> Optional[int]:
        ...
        self.console.print("This router can send execution down different paths based on conditions.\n")
        ...
        selection = inquirer.select(
            message="Which branch would you like to explore?",
            choices=choices
        ).execute()
    ...
    def _follow_branch(self, branch_index: int):
        ...
        if not branch_modules:
            self.console.print("[yellow]This branch has no modules. Continuing to next main flow module.[/yellow]")
    ...
    def _live_walkthrough_branch(self, branch_modules: List[Dict[str, Any]], branch_name: str):
        ...
        while branch_step_index < len(branch_steps):
            try:
                ...
                # Display branch step
                self._display_branch_step(current_branch_step, branch_step_index, len(branch_steps), branch_name)
                
                # Get branch action via keyboard input or fallback to menu
                if self.keyboard_available:
                    action = self._handle_keyboard_input_branch(branch_step_index, len(branch_steps))
                else:
    ...
    def _display_branch_step(self, step: FlowStep, step_index: int, total_steps: int, branch_name: str):
        ...
        panel = Panel(
            content,
            title=f"Branch Module {step.module_id}",
            border_style="cyan",
            expand=False
        )
    ...
    def _get_branch_action(self, step_index: int, total_steps: int) -> str:
        ...
        return inquirer.select(
            message="What would you like to do in this branch?",
            choices=choices
        ).execute()
    ...
    def _handle_keyboard_input(self) -> str:
        ...
        while True:
            ...
            # Letter keys
                ...
                self.console.print(f"[yellow]Unknown key '{key}'. Use arrow keys or 'm' for menu.[/yellow]")
                continue
    ...
    def _handle_keyboard_input_branch(self, step_index: int, total_steps: int) -> str:
        ...
        while True:
            ...
            # Letter keys
                ...
                # Show help for invalid keys
                self.console.print(f"[yellow]Unknown key '{key}'. Use arrow keys, 'd' for details, 'q' to exit branch, or 'm' for menu.[/yellow]")
                continue
