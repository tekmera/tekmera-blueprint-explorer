...
def _classify_make_module_type(module_type: str) -> str:
    ...
    if any(keyword in module_lower for keyword in ["trigger", "webhook", "watch", "listen", "monitor"]):
        return "triggers_and_sources"
    
    # Data processing
    elif any(keyword in module_lower for keyword in ["parse", "json", "csv", "xml", "aggregate", "iterator", "transform"]):
        return "data_processing"
    
    # Integrations (app-specific modules)
    elif any(keyword in module_lower for keyword in ["google", "microsoft", "slack", "notion", "airtable", "shopify", "stripe"]):
        return "integrations"
    
    # Communications
    elif any(keyword in module_lower for keyword in ["email", "sms", "notification", "message", "chat"]):
    ...
    # Utilities
    elif any(keyword in module_lower for keyword in ["tool", "delay", "variable", "set", "get", "http"]):
        return "utilities"
...
def _get_make_module_category(module_type: str) -> str:
    ...
    # Utility patterns
    elif any(keyword in module_lower for keyword in ["tool", "router", "filter", "delay", "variable", "set"]):
        return "utility"
...
def _analyze_make_patterns(module_types: List[str]) -> Dict[str, Any]:
    ...
    integration_modules = []
    integration_services = ["google", "microsoft", "slack", "notion", "airtable", "shopify", "stripe", "trello", "asana"]
    for module_type in module_types:
    ...
    data_processing_ratio = (len(json_modules) + len(iterator_modules) + len(aggregator_modules)) / len(module_types) if module_types else 0
