...
class SearchInterface:
    ...
    def start(self, directory: Path):
        ...
        self.console.print(f"✅ Loaded {stats['total_blueprints']} blueprints with {stats['total_modules']} total modules\n")
    ...
    def _show_connections(self):
        ...
        for conn_id, usages in connections.items():
            connection_label = connection_labels.get(int(conn_id) if str(conn_id).isdigit() else conn_id, f"Connection {conn_id}")
...
[bold]Scenarios:[/bold]"""
    ...
    def _display_text_search_results(self, search_text: str, matches: List[Dict], case_sensitive: bool):
    ...
    def _show_text_results_page(self, page_matches: List[Dict], search_text: str, current_page: int, total_pages: int):
