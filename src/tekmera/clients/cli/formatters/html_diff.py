...
class HTMLDiffFormatter(BaseFormatter):
    ...
    def render(self, report: BaseReport) -> str:
        ...
        if not isinstance(report, BlueprintDiffReport):
            raise ValueError(
                f"HTMLDiffFormatter can only render BlueprintDiffReport, got {type(report)}"
            )
    ...
    def _generate_html(self, report_data) -> str:
        ...
        # Extract structured data from repor
...
def render_report_to_html(report_data: Any, output_path: str) -> str:
    ...
    html_content = _generate_html(
        summary_data, overview_data, component_groups, connection_analysis
    )
...
def extract_summary(report_data: Any, connection_analysis: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    if hasattr(report_data, "summary") and hasattr(report_data, "module_changes"):
        ...
        changed_components = [
            c for c in report_data.module_changes if c.change_type.value != "unchanged"
        ]
...
def extract_component_groups(report_data: Any) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
    ...
    for comp_type in all_component_types:
        ...
        groups[comp_type] = {"component_id": comp_id, "added": [], "updated": [], "removed": []}
    ...
    if hasattr(report_data, "module_changes"):
        ...
        for change in changed_components:
            ...
            change_data = {
                ...
                "impact_description": change.description,
...
def _get_component_type(change, platform):
    ...
    # Prioritize component type over change conten
...
def _generate_html(
    ...
    connection_analysis: Dict[str, Any] = None,
) -> str:
    ...
    connection_block = (
        _generate_connection_summary_standalone(connection_analysis)
        if connection_analysis and connection_analysis.get("has_connection_changes")
        else ""
    )
...
def _generate_sidebar(
    component_groups: Dict[str, Dict[str, Any]], connection_analysis: Dict[str, Any] = None
) -> str:
...
def _generate_summary_section(summary_data: Dict[str, Any]) -> str:
    ...
    # Format categories as inline tex
            ...
            if comp_data["added"]:
                ...
                for item in comp_data["added"]:
                    details_content.append(
                        f"                                <li>{_format_component_item(item, comp_type)}</li>"
                    )
                details_content.append("                            </ul>")
...
def _generate_connection_summary_section(connection_analysis: Dict[str, Any]) -> str:
    ...
    # Calculate total connection changes for header coun
    ...
    if replacement_count > 0:
        ...
        total_modules_affected = sum(
            repl["module_count"] for repl in connection_analysis.get("replacements", [])
        )
        count_parts.append(
            f"{replacement_count} replacement pattern(s) affecting {total_modules_affected} modules"
        )
    if new_count > 0:
    ...
    if replacements:
        ...
        for replacement in replacements:
            ...
            content.append(
                "                            <p><strong>Connection replacement across {} module(s):</strong></p>".format(
                    module_count
                )
            )
            content.append('                            <div class="connection-details">')
            content.append(
                f'                                <div><strong>From:</strong> {old_display} <span class="connection-id">(ID: {old_id})</span></div>'
            )
            content.append(
                f'                                <div><strong>To:</strong> {new_display} <span class="connection-id">(ID: {new_id})</span></div>'
            )
            content.append("                            </div>")
            ...
            for module in replacement["modules"]:
                ...
                content.append(
                    f'                                <li>Module {module_id}: "{module_name}"</li>'
                )
    ...
    if new_count or removed_count or isolated_count:
        ...
        if new_count > 0:
            content.append(
                f"                            <li>{new_count} module(s) got new connections</li>"
            )
        if removed_count > 0:
            content.append(
                f"                            <li>{removed_count} module(s) lost connections</li>"
            )
        if isolated_count > 0:
            content.append(
                f"                            <li>{isolated_count} isolated connection change(s)</li>"
            )
        content.append("                        </ul>")
...
def _format_component_item(item: Dict[str, Any], comp_type: str) -> str:
    ...
    if change_detail:
        ...
        if comp_type == "Modules" and (
            '<div class="module-changes-cards">' in change_detail
            or '<div class="module-changes-table">' in change_detail
        ):
            ...
            # For simple changes, keep the original forma
...
def _get_standardized_change_description(item: Dict[str, Any], comp_type: str) -> str:
    ...
    if not item.get("raw_data_before"):
        # Use the detailed description from structural analysis if available
        return item.get("impact_description", "Added to workflow")
...
def _display_configuration_changes(config_changes: List[Dict]) -> str:
    ...
    descriptions = []
    for change in config_changes:  # Show ALL changes, no limi
...
def _detect_and_format_change_value(old_val, new_val, field) -> str:
    """Detect and format a change between old and new values."""
    old_formatted = _format_change_value(old_val)
    new_formatted = _format_change_value(new_val)
    return f"{field}: {old_formatted} → {new_formatted}"
    ...
    for change in config_changes:
        if isinstance(change, dict):
            ...
            if (
                ...
                and description != "Error handler configuration changed"
            ):
...
def _generate_change_summary(item: Dict[str, Any], change_detail: str) -> str:
    ...
    # Count the number of change cards for field coun
...
def _format_card_values_with_diff(old_value, new_value) -> tuple[str, str]:
    ...
    if isinstance(old_value, str) and isinstance(new_value, str):
        ...
        return (
            f'<em class="string-value">{old_highlighted}</em>',
            f'<em class="string-value">{new_highlighted}</em>',
        )
    else:
...
def _extract_filter_details(item: Dict[str, Any]) -> str:
    ...
    previous_filter = (
        raw_data_before.get("filter", {}) or raw_data_before.get("parameters", {}).get("filter", {})
        if raw_data_before
        else {}
    )
    ...
    if current_filter and previous_filter:
        ...
        if current_conditions and previous_conditions:
            ...
            current_first = (
                current_conditions[0][0] if current_conditions and current_conditions[0] else {}
            )
            previous_first = (
                previous_conditions[0][0] if previous_conditions and previous_conditions[0] else {}
            )
