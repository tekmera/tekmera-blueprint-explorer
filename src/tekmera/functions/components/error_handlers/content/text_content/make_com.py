"""Make.com error handler text content extraction."""
...
def text_content(error_handler_component: ErrorHandlerComponent) -> ModuleResult[str]:
    """
    Extract text content from Make.com error handler component.

    Similar to Workfront Fusion but with Make.com specific patterns:
    - Focus on practical retry configurations
    - Simpler error handling structures
    - More direct parameter mappings
    """
    text_parts = []
...
def _extract_error_handler_text(handler: Dict[str, Any], handler_number: int) -> List[str]:
    """Extract text from individual error handler module (Make.com patterns)."""
    ...
    # Extract from mapper (Make.com often uses more direct mappings)
    ...
    if isinstance(mapper, dict):
        ...
        # Make.com specific error handling fields
        if "errorType" in mapper:
            error_type = mapper["errorType"]
            text_parts.append(f"Error Type: {error_type}")

        if "message" in mapper:
            message = mapper["message"]
            if isinstance(message, str) and message.strip():
                text_parts.append(f"Error Message: {message}")

        # Other mapper fields
        for key, value in mapper.items():
            if key not in ["retry", "count", "interval", "errorType", "message"]:
