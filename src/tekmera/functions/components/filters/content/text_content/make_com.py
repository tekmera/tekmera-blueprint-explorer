"""Make.com filter text content extraction."""
...
def text_content(filter_component: FilterComponent) -> ModuleResult[str]:
    """
    Extract text content from Make.com filter component.

    Similar to Workfront Fusion but with Make.com specific patterns:
    - Focus on variable expressions and functions
    - Simpler condition structure
    - More direct value comparisons
    """
    text_parts = []
...
def _extract_filter_conditions(filter_data: Dict[str, Any]) -> List[str]:
    """Extract text from filter condition structure (Make.com patterns)."""
    text_parts = []
                ...
                for condition_idx, condition in enumerate(condition_group):
                    if isinstance(condition, dict):
                        ...
                        if a and isinstance(a, str):
                            # Make.com often uses function expressions
                            text_parts.append(f"  Left Expression: {a}")
                        ...
                        if b and isinstance(b, str):
                            ...
                            text_parts.append(f"  Right Value: {str(b)}")
