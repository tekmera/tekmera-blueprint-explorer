...
def _classify_workfront_module_type(module_type: str) -> str:
    ...
    # External triggers and watchers
        ...
        keyword in module_lower for keyword in ["watch", "trigger", "webhook", "listen", "monitor"]
    ):
    ...
    # Data transformation
        ...
        for keyword in ["parse", "json", "csv", "xml", "convert", "transform"]
    ):
    ...
    # External actions
        ...
        keyword in module_lower
        for keyword in ["http", "email", "slack", "teams", "adobe", "sftp", "ftp"]
    ):
    ...
    # Flow control
        ...
        keyword in module_lower
        for keyword in ["router", "filter", "iterator", "aggregator", "branch"]
    ):
        return "flow_control"
    ...
    # Utilities
        ...
        keyword in module_lower
        for keyword in ["util", "tool", "variable", "set", "delay", "datastore"]
    ):
        return "utilities"
...
def _get_workfront_module_category(module_type: str) -> str:
    ...
    # Utility patterns
        ...
        keyword in module_lower
        for keyword in ["util", "tool", "variable", "set", "router", "filter", "delay"]
    ):
        return "utility"
...
def _analyze_workfront_patterns(module_types: List[str]) -> Dict[str, Any]:
    ...
    webhook_modules = [
        t for t in module_types if "webhook" in t.lower() or "customwebhook" in t.lower()
    ]
    ...
    has_workfront_core = any(
        "workfront:search" in t.lower()
        or "workfront:custom" in t.lower()
        or "workfront:watch" in t.lower()
    ...
    external_dependency_ratio = (
        (len(webhook_modules) + len(email_modules) + len(sftp_modules)) / len(module_types)
        if module_types
        else 0
    )
    ...
    return {
        ...
        "complexity_indicators": {
            ...
            "multi_system_integration": len(
                set([t.split(":")[0] for t in module_types if ":" in t])
            )
            > 3,
