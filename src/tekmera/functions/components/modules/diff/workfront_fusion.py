...
def analyze_workfront_fusion_module(
...
def _analyze_workfront_parameters(
    ...
    for key in all_keys:
        ...
        if old_value != new_value:
            ...
            if (
                change_type == "modified"
                and _is_json_field(old_value)
                and _is_json_field(new_value)
            ):
                # Parse JSON and return individual field differences
                json_differences = _analyze_json_field_changes(
                    old_value, new_value, f"parameters.{key}"
                )
                ...
                description = _generate_workfront_parameter_description(
                    key, old_value, new_value, change_type
                )
...
def _analyze_workfront_mapper(
    old_mapper: Dict[str, Any], new_mapper: Dict[str, Any]
) -> List[ModuleDifference]:
                ...
                if field not in old_mapper:
                    ...
                    try:
                        if _is_json_field(old_value) and _is_json_field(new_value):
                            json_differences = _analyze_json_field_changes(
                                old_value, new_value, f"mapper.{field}"
                            )
                            if json_differences:
                                differences.extend(json_differences)
                            else:
...
def _analyze_workfront_metadata(
...
def _assess_workfront_parameter_significance(
    param_name: str, old_value: Any, new_value: Any
) -> str:
    ...
    # Cosmetic parameters
    elif param_lower in ["name", "label", "description", "notes", "comment"]:
        return "cosmetic"
...
def _generate_workfront_parameter_description(
    param_name: str, old_value: Any, new_value: Any, change_type: str
) -> str:
...
def _analyze_json_field_changes(
    old_json: str, new_json: str, field_prefix: str
) -> List[ModuleDifference]:
