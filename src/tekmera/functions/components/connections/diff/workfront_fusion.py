...
def analyze_workfront_fusion_connection(
    ...
    if old_conn_id != new_conn_id:
        ...
        differences.append(
            ConnectionDifference(
                ...
                change_type=(
                    "modified"
                    if old_conn_id and new_conn_id
                    else ("removed" if old_conn_id else "added")
                ),
...
def _extract_workfront_user(label: str) -> str:
    ...
    for pattern in user_patterns:
        ...
        if match:
            ...
            if not any(
                keyword in user.lower()
                for keyword in ["connection", "prod", "test", "staging", "sa"]
            ):
                return user
