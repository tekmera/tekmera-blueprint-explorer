"""Make.com blueprint text search implementation."""
...
def text_content(
    ...
    Now properly detects routers, filters, and error handlers in Make.com blueprints.
    """
...
def _search_single_blueprint(
    blueprint: Dict[str, Any], query: str, case_sensitive: bool
) -> Dict[str, Any]:
    """Search within a single Make.com blueprint."""
    # Step 1: Extract all typed components (now includes routers, filters, error handlers)
    ...
    # Search routers (now properly detected in Make.com)
    for router_component in all_components["routers"]:
        try:
            text_result = router_text_content(router_component, Platform.MAKE_COM)
            text_content = text_result.data
    ...
    for error_handler_component in all_components["error_handlers"]:
        try:
            text_result = error_handler_text_content(error_handler_component, Platform.MAKE_COM)
            text_content = text_result.data
