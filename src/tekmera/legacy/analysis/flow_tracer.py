...
class FlowTracer:
    ...
    def trace_scenario(
        self, blueprint_data: Dict[str, Any], output_format: str = "tree"
    ) -> List[TraceStep]:
        """
    ...
    def _trace_from_module(
        self,
        module: Dict[str, Any],
        flow_context: List[Dict],
        depth: int = 0,
        route_name: Optional[str] = None,
        filter_condition: Optional[str] = None,
    ):
    ...
    def _trace_next_module(
        self, current_module: Dict[str, Any], flow_context: List[Dict], depth: int
    ):
    ...
    def _format_step_text(self, step: TraceStep, include_emoji: bool = True) -> Text:
        ...
        if step.de_fields:
            annotations.append(
                f"DE fields: {', '.join(step.de_fields[:2])}{'...' if len(step.de_fields) > 2 else ''}"
            )
