...
def get_router_routes(router_module):
    ...
    if "routes" in router_module:
        for route in router_module["routes"]:
            routes.append({"flow": route.get("flow", []), "filter": route.get("filter", {})})
    return routes
...
def main():
    ...
    if router_v17 and router_v18:
        ...
        if email_in_route_v17 or email_in_route_v18:
            ...
            console.print(
                f"  v17: Route {route_idx_v17}, Module present: {email_in_route_v17 is not None}"
            )
            console.print(
                f"  v18: Route {route_idx_v18}, Module present: {email_in_route_v18 is not None}"
            )
