...
class LemonSqueezyError(Exception):
    ...
    def __init__(
        self, message: str, status_code: Optional[int] = None, error_data: Optional[Dict] = None
    ):
...
class LemonSqueezyAPI:
    ...
    def __init__(self, config: Optional[LemonSqueezyConfig] = None):
        ...
        self.session.headers.update(
            {"Accept": "application/json", "User-Agent": "Tekmera-Fusion-Explorer/1.0"}
        )
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                raise LemonSqueezyError(
                    message=error_message, status_code=response.status_code, error_data=error_data
                )
    ...
    def activate_license(
        self, license_key: str, instance_name: Optional[str] = None
