...
from tekmera.functions.meta.types import Platform, ProjectionResult
...
class BlueprintDiffReport(BaseReport):
    ...
    def _get_linked_module_name(self, module_id: str, all_changes: List[ModuleChange]) -> str:
        ...
        for change in all_changes:
            if change.module_id == module_id and not change.module_id.endswith(
                ("_filter", "_router", "_error")
            ):
    ...
    def _get_filter_relationship(
        self, filter_change: ModuleChange, all_changes: List[ModuleChange]
    ) -> str:
    ...
    def _get_filter_condition_summary(self, filter_change: ModuleChange) -> str:
        ...
        try:
            ...
            total_conditions = sum(
                len(group) if isinstance(group, list) else 1 for group in conditions
            )
    ...
    def _get_filter_text_content(self, filter_change: ModuleChange) -> List[str]:
        ...
        try:
            ...
            elif filter_change.change_type in [
                ChangeType.CONFIGURATION_CHANGED,
                ChangeType.STRUCTURALLY_MOVED,
            ]:
                # For updated filters, show before and after
                if filter_change.raw_data_before:
                    # Show before state
                    before_content = self._extract_filter_conditions_text(
                        filter_change.raw_data_before
                    )
                    if before_content:
...
def generate_diff_report(
    ...
    if platform1 != platform2:
        raise ValueError(
            f"Platform mismatch: {platform1.value} vs {platform2.value}. Cannot compare blueprints from different platforms."
        )
