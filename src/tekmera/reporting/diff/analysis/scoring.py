...
from ..diff import ChangeScale, ChangeType, ModuleChange
from .structural import GraphComparisonResult


def calculate_structural_change_score(
    graph1: TopologyGraph, graph2: TopologyGraph, comparison: GraphComparisonResult
) -> float:
    """
...
def classify_change_scope(
    change_score: float, module_changes: List[ModuleChange], comparison: GraphComparisonResult
) -> ChangeScale:
    """
...
def _calculate_edge_change_score(
    ...
    edge_change_count = added_edges + removed_edges + changed_edges
