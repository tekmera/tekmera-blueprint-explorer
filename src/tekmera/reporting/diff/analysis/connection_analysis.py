...
def analyze_connection_changes(
    module_changes: List[ModuleChange], platform: str
) -> ConnectionSummary:
    """
...
def _extract_connection_changes(
    module_changes: List[ModuleChange], platform: str
) -> List[Dict[str, Any]]:
...
def _extract_connection_change_from_module(
    change: ModuleChange, platform: str
) -> Dict[str, Any] | None:
    ...
    for config_change in config_changes:
        ...
        if platform.lower() == "workfront_fusion":
            ...
            is_connection_change = (
                "account" in field or "make.com connection changed" in description.lower()
            )
        else:
...
def _group_connection_replacements(
...
def _create_replacement_summaries(
    replacement_groups: Dict[Tuple[str, str], List[Dict[str, Any]]], platform: str
) -> List[ConnectionReplacement]:
    ...
    for (old_id, new_id), modules in replacement_groups.items():
        ...
        old_display, new_display = _extract_connection_displays(
            modules[0]["description"], old_id, new_id
        )
...
def _identify_isolated_changes(
    connection_changes: List[Dict[str, Any]],
    ...
    for change in connection_changes:
        ...
        if module_id in grouped_module_ids or not old_id or not new_id:
            continue
...
def format_connection_summary_for_html(summary: ConnectionSummary) -> Dict[str, Any]:
    ...
    return {
        "has_connection_changes": bool(
            summary.replacements
            or summary.new_connections
            or summary.removed_connections
            or summary.isolated_changes
        ),
        "replacements": [
            {
