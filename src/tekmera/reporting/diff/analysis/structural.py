...
def detect_node_changes(comparison: GraphComparisonResult) -> List[ModuleChange]:
    ...
    for old_node, new_node in comparison.modified_nodes:
        ...
        if new_node.is_filter and 'source_router_id' in new_node.raw_data:
            ...
            component_metadata = {'source_router_id': getattr(new_node.raw_data, 'source_router_id')}
...
def _match_nodes_between_graphs(graph1: TopologyGraph, graph2: TopologyGraph) -> List[Tuple[TopologyNode, TopologyNode]]:
    """
...
def _has_configuration_changed(node1: TopologyNode, node2: TopologyNode) -> bool:
    ...
    if (node1.is_trigger != node2.is_trigger or
        node1.is_router != node2.is_router or
        node1.is_filter != node2.is_filter or
        node1.is_error_handler != node2.is_error_handler):
        return True
...
def _has_position_changed(node1: TopologyNode, node2: TopologyNode) -> bool:
    ...
    return (node1.position.path != node2.position.path or 
            node1.position.depth != node2.position.depth)
    ...
    for key in all_keys:
        ...
        if old_value != new_value:
            ...
            if isinstance(old_value, dict) and isinstance(new_value, dict):
                nested_changes = _compare_dict_fields(old_value, new_value, field_name, max_depth - 1)
