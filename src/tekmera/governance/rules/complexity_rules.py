...
def check_flow_complexity_index(
    ...
    violations.append(
        GovernanceViolation(
            ...
            suggested_fix=(
                "Consider breaking this scenario into smaller, more focused scenarios or reducing nesting levels."
                if is_violation
                else "Scenario complexity is well-managed."
            ),
...
def check_router_density_analysis(
    blueprint_data: Dict[str, Any], scenario_name: str
) -> List[GovernanceViolation]:
    ...
    violations.append(
        GovernanceViolation(
            ...
            suggested_fix=(
                "Consider consolidating router logic or restructuring the flow to reduce decision complexity."
                if is_violation
                else "Router distribution is appropriate."
            ),
            rule_description="Measures the percentage of modules that are routers (decision points) in the scenario. "
            "High router density indicates excessive branching logic, which can make scenarios harder to follow, "
            "more prone to logical errors, and difficult to maintain. Scenarios with too many decision points "
...
def check_route_fan_out_profile(
    blueprint_data: Dict[str, Any], scenario_name: str
) -> List[GovernanceViolation]:
...
def check_flow_depth_estimate(
    ...
    violations.append(
        GovernanceViolation(
            ...
            suggested_fix=(
                "Consider breaking the scenario into smaller sub-scenarios or using parallel processing where appropriate."
                if is_violation
                else "Flow depth is manageable."
            ),
