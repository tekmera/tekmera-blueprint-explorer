...
class CorpusAnalyzer:
    ...
    def get_de_field_rankings(self) -> List[Tuple[str, int, List[Dict]]]:
        ...
        ranked_fields = [(field, len(usages), usages) 
    ...
    def get_module_type_rankings(self) -> List[Tuple[str, int, List[Dict]]]:
        ...
        # Sort by usage count (descending)
        ranked_types = [(module_type, len(usages), usages) 
                       for module_type, usages in type_usage.items()]
        ...
        for i, field1 in enumerate(all_fields):
            ...
            if len(similar_fields) > 1:
                inconsistencies.append({
                    ...
                    'similarity_scores': [
                        SequenceMatcher(None, similar_fields[0][3:].lower(), field[3:].lower()).ratio()
    ...
    def analyze_connections(self) -> Dict[str, Any]:
        ...
        for blueprint_file, blueprint_data in self.corpus_data.items():
            ...
            blueprint_analysis = analyzer.analyze_blueprint_connections(blueprint_data, scenario_name)
            ...
            for conn_id, usages in blueprint_analysis['connections'].items():
                for usage in usages:
                    ...
                    connection_info = {
                        ...
                        'connection_label': blueprint_analysis['connection_labels'].get(conn_id, f"Connection {conn_id}"),
    ...
    def _generate_connection_warnings(self, connection_types: Dict[str, Set[int]], connection_labels: Dict[int, str]) -> List[Dict[str, Any]]:
        """
