...
class ConnectionAnalyzer:
    ...
    def determine_connection_type(self, module_type: str, connection_label: str = "", context: str = "") -> str:
        """
    ...
    def analyze_blueprint_connections(self, blueprint_data: Dict[str, Any], scenario_name: str) -> Dict[str, Any]:
        """
...
def display_connection_table(
    ...
    for conn_id, usages in sorted(connections.items()):
        ...
        row_data.extend([str(len(unique_module_ids)), module_id_text, scenario_text])  # Use unique count, no module_text
...
def display_connection_warnings(console: Console, warnings: List[Dict[str, Any]]):
    ...
    for warning in warnings:
        ...
        if 'connection_details' in warning:
            content_lines.append(f"[bold]Connections:[/bold] {', '.join(warning['connection_details'])}")
