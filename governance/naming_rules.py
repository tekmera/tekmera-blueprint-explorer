...
def check_scenario_naming_prefix(blueprint_data: Dict[str, Any], scenario_name: str) -> List[GovernanceViolation]:
    ...
    if has_valid_prefix:
        matched_prefix = next(prefix for prefix in valid_prefixes if scenario_name.startswith(prefix))
...
def check_default_module_labels(blueprint_data: Dict[str, Any], scenario_name: str) -> List[GovernanceViolation]:
    ...
    if total_modules == 0:
        violations.insert(0, GovernanceViolation(
            rule_id="GOV-NAME-002",
            rule_title="Default Module Label",
