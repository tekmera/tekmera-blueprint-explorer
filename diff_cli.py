...
class FusionDiff:
    ...
    def _compare_blueprints(self, blueprint1: Dict, blueprint2: Dict):
        ...
        comparison_table.add_row(
            "Scenario Name",
            blueprint1['scenario_name'],
            blueprint2['scenario_name']
        )
        comparison_table.add_row(
            "Total Modules",
            str(len(modules1)),
        ...
        self._show_field_differences(de_fields1, de_fields2, blueprint1['filename'], blueprint2['filename'])
        
        # Show module type differences
        self._show_module_type_differences(modules1, modules2, blueprint1['filename'], blueprint2['filename'])
        
        # Show detailed module differences
        modified_modules = self._show_module_differences(modules1, modules2, blueprint1['filename'], blueprint2['filename'])
    ...
    def _show_field_differences(self, fields1: Set[str], fields2: Set[str], name1: str, name2: str):
        ...
        if only_in_1 or only_in_2:
            ...
            if only_in_1:
                ...
                panel = Panel(panel_content, title=f"Only in {name1} ({len(only_in_1)} fields)", 
                            border_style="red", expand=False)
                self.console.print(panel)
            
            if only_in_2:
                panel_content = "\n".join(f"• {field}" for field in sorted(only_in_2))
                panel = Panel(panel_content, title=f"Only in {name2} ({len(only_in_2)} fields)", 
    ...
    def _show_module_differences(self, modules1: List, modules2: List, name1: str, name2: str):
        ...
        if modified_modules:
            self.console.print(f"[yellow]📝 Functionally modified modules ({len(modified_modules)}):[/yellow]")
        ...
        if common_ids and not modified_modules:
            self.console.print(f"[green]✓ All {len(common_ids)} common modules are identical[/green]\n")
    ...
    def _identify_functional_changes(self, mod1: Dict, mod2: Dict) -> List[str]:
        ...
        if (mod1.get('module') == 'builtin:BasicRouter' or 
            mod2.get('module') == 'builtin:BasicRouter'):
    ...
    def _analyze_router_changes(self, mod1: Dict, mod2: Dict) -> List[str]:
        ...
        for i in range(max_routes):
            ...
            route_changes = self._analyze_route_flow_changes(route1.get('flow', []), 
                                                           route2.get('flow', []), i+1)
    ...
    def _analyze_route_flow_changes(self, flow1: List, flow2: List, route_num: int) -> List[str]:
        ...
        for i in range(max_modules):
            ...
            filter_changes = self._analyze_filter_changes(mod1.get('filter'), 
                                                         mod2.get('filter'), 
                                                         route_num, i+1)
    ...
    def _analyze_filter_changes(self, filter1: Optional[Dict], filter2: Optional[Dict], 
                               route_num: int, module_num: int) -> List[str]:
    ...
    def _show_change_summary(self, modified_modules: List, name1: str, name2: str):
        ...
        for module_change in module_changes:
            ...
            for router_id, contained_ids in related_changes.items():
                if mod_id in contained_ids:
                    ...
                    router_change = next((rc for rc in router_changes if rc['id'] == router_id), None)
                    if router_change:
        ...
        self.console.print(f"[bold cyan]Total functional changes: {total_functional_changes}[/bold cyan]\n")
        ...
        for i, grouped in enumerate(grouped_changes, 1):
            ...
            self.console.print(f"   📍 Affected Module: {module['name']} ({module['type']}, ID: {module['id']})")
        ...
        for i, change in enumerate(standalone_changes, len(grouped_changes) + 1):
            ...
            self.console.print(f"   📍 Module: {change['name']} ({change['type']}, ID: {change['id']})")
            self.console.print(f"   🔄 Nature of Change:")
        ...
        standalone_routers = [rc for rc in router_changes 
    ...
    def _analyze_mapper_changes(self, mapper1: Dict, mapper2: Dict) -> List[str]:
        ...
        if added_keys:
            changes.append(f"Mapper: {len(added_keys)} field(s) added ({', '.join(added_keys[:3])}{'...' if len(added_keys) > 3 else ''})")
        
        if removed_keys:
            changes.append(f"Mapper: {len(removed_keys)} field(s) removed ({', '.join(removed_keys[:3])}{'...' if len(removed_keys) > 3 else ''})")
        ...
        if changed_keys:
            ...
            if content_changes:
                ...
                changes.append(f"Mapper: {len(changed_keys)} field(s) modified ({', '.join(changed_keys[:3])}{'...' if len(changed_keys) > 3 else ''})")
    ...
    def _analyze_parameter_changes(self, params1: Any, params2: Any) -> List[str]:
        ...
        if isinstance(params1, dict) and isinstance(params2, dict):
            ...
            if changed_keys:
                changes.append(f"Parameters: {len(changed_keys)} setting(s) modified ({', '.join(changed_keys[:3])}{'...' if len(changed_keys) > 3 else ''})")
        else:
